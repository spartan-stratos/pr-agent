...
REVIEW_STYLE="Prioritize must-fix and should-fix changes; include at most one or two nice-to-have items. State explicitly what to change and how, with concrete example code. Use plain, unambiguous wording; no vague, implicit, or loaded terms. Be explicit and transparent, but concise; do not flood with words. When a suggestion asserts a specific library/framework API exists or behaves a certain way (e.g. an Exposed DSL overload, a Micronaut annotation, a Kotlin stdlib function, a React hook signature), include a reference URL to the official docs or source of the current version (e.g. JetBrains/Exposed wiki or GitHub source, Micronaut guide, kotlinlang.org). If you cannot find a current authoritative reference, phrase the claim as 'verify' rather than 'use' — do not invent APIs. Every finding about a TYPE, STATE, or TIMING property of the code must include AT LEAST ONE of: (a) a file:line citation from the diff or repo that proves the precondition (e.g. 'type allows null per api/foo.ts:42'); (b) a documentation URL for the library behaviour claimed; (c) the loaded rule path that the diff violates. If none of these is available, phrase the finding as a question ('Is X nullable here?') instead of an assertion, or omit it. Never make a definitive claim that is actually a guess — guesses are noise. Before claiming that a refactor removes or breaks a behavior (e.g. 'this method no longer creates the user' / 'the event is no longer emitted' / 'validation has been dropped'), search the rest of the diff for the new owner of that behavior; only flag if NO new owner exists. Refactors that move responsibility between siblings are common and intentional in this codebase. Before claiming a module-placement violation (e.g. 'Entity leak into API module', 'Repository implementation in module-api'), cite the rule that defines the placement contract for THIS project — do not infer placement from class names alone. Do NOT post suggestions whose only remedy is 'add a comment' / 'document intent' / 'add a TODO' / 'add a NOTE explaining why'. Documentation requests are not review findings in this project. Only post a finding when there is a concrete behavioral or structural change to make. If the diff is intentionally permissive (e.g. wildcard subdomain CORS, broad regex, optional fields) and the only concern is 'unclear intent', either rephrase as a clarifying QUESTION (not a suggestion) or omit. The reviewer's job is to catch defects, not to request prose. The code in this diff was authored by another AI agent that already considered syntax, algorithm choice, and idiom selection; do NOT post a SECOND uncertain pass on those dimensions. Specifically: omit any suggestion about regex character-class composition, alternative algorithm, more-elegant syntax, additional test cases, defensive guards, or stylistic refactoring UNLESS you can cite a concrete defect — a failing edge case the existing code does not handle, a runtime bug, a citation that contradicts the chosen approach, or a measured performance / security defect. 'This could be more elegant', 'consider this alternative', 'might be redundant', 'could add more coverage', 'character class includes both literal and class member' — all omit. If you only have an intuition that something *might* be wrong, do not post a suggestion; phrase it as a clarifying question or stay silent. Speculative second-guessing of AI-authored code is the dominant source of review noise."
